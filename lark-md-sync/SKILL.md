...
Model: `tmp/lark-md-sync/state.json` stores tracked Markdown mappings and the optional current Lark target. Local tracked `.md` files carry `lark_url` / `lark_file_token` in frontmatter for easy inspection. `tmp/lark-md-sync/base/` stores three-way merge bases. Default sync operations are dry-run; local or remote content writes require `--apply`.
